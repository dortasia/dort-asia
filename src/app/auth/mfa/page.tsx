...
import { verifyRecoveryCode } from "@/app/dashboard/settings/security/recovery-actions";
  ...
  const submitCode = async (codeToVerify: string) => {
    ...
    if (isRecoveryMode) {
      ...
      try {
        ...
        if (res.success && res.action === 'reauthenticate') {
          router.push("/auth?recovery=true");
        } else if (res.success) {
          router.push("/auth/mfa/recover");
        } else {
    ...
    if (!factorId || codeToVerify.length !== 6) return;
  ...
  return (
    ...
    >
      ...
      <h1 className="text-[23px] sm:text-[25px] font-semibold text-gray-900 dark:text-white tracking-tight mb-2">
        {isRecoveryMode ? "Use Recovery Code" : "Two-Step Verification"}
      </h1>
      <p className="text-[14px] text-gray-500 dark:text-gray-400 max-w-[310px] leading-relaxed mb-7">
        {isRecoveryMode 
          ? "Enter one of your 8-character recovery codes to regain access to your account."
      ...
      <form onSubmit={handleManualSubmit} className="w-full flex flex-col items-center">
        {isRecoveryMode ? (
          <div className="w-full mb-6 max-w-[340px]">
            <input
              ...
              className="w-full h-14 rounded-xl border border-gray-300 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-center font-mono text-[20px] font-semibold text-gray-900 dark:text-white tracking-widest focus:border-[#0071E3] focus:ring-4 focus:ring-[#0071E3]/15 transition-all outline-none uppercase placeholder-gray-300"
            />
          </div>
        ) : (
        ...
        <div className="w-full space-y-3 pt-1">
          <button
            type="submit"
            disabled={(isRecoveryMode ? recoveryCode.length < 8 : !isComplete) || isVerifying}
          ...
          <button
            ...
            className="w-full text-[13.5px] font-medium text-[#0071E3] hover:text-[#0077ED] dark:text-[#0A84FF] transition-colors cursor-pointer py-1"
          >
            {isRecoveryMode ? "Use authenticator app" : "Lost authenticator?"}
          </button>

          <button
            type="button"
            onClick={handleSignOut}
            className="w-full text-[13.5px] font-medium text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors cursor-pointer py-1"
          >
            Cancel and sign out
          </button>
        </div>
