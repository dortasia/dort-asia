  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 pointer-events-none font-text">
      <div 
        className={`absolute inset-0 bg-gradient-to-b ${
          theme === "dark" 
            ...
            : "from-white/55 via-white/25 to-transparent"
        } h-[120px] -z-10 transition-colors duration-300`} 
      />
      <div className="w-full flex items-center justify-between h-14 relative pointer-events-auto pt-[20px] px-6 md:px-10">
        ...
        {/* Desktop Navigation - Frosted Dark Grey Glossy Pill Bar with Sliding Active Indicator */}
              ...
              <>
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-white/20 rounded-full border border-white/25 shadow-[inset_0_1px_1px_rgba(255,255,255,0.25)]"
                ...
                <HugeiconsIcon 
                  ...
                  className={`w-5 h-5 relative z-10 transition-all ${
                    isActive ? "text-white scale-105" : "text-zinc-300 group-hover:text-white group-hover:scale-105"
                  }`} 
                />
                <span className={`text-[14px] font-medium tracking-tight relative z-10 transition-colors ${
                ...
                {link.hasDropdown && (
                  <HugeiconsIcon
                    ...
                    className={`w-4 h-4 relative z-10 transition-transform ${
                      activeDropdown === link.name ? "rotate-180 text-white" : "text-zinc-400 group-hover:text-white"
                    }`}
                  />
                )}
              </>
        ...
        <div className="hidden lg:flex items-center gap-4">
          <motion.button
            ...
            initial={false}
            animate={{
              paddingLeft: isAuthenticated === null ? "6px" : "24px",
              paddingRight: "6px",
            }}
            transition={{
              type: "spring",
            ...
            className="group rounded-full bg-[#2b7fff] hover:bg-[#1a6eff] text-white font-medium text-[14.5px] tracking-tight py-1.5 h-[44px] flex items-center justify-center gap-3 transition-colors shadow-none overflow-hidden cursor-pointer disabled:cursor-default"
          >
            <AnimatePresence mode="popLayout" initial={false}>
              {isAuthenticated !== null && (
                <motion.span
                  key={isAuthenticated ? "dashboard" : "get-started"}
                  initial={{ opacity: 0, width: 0, x: -12, filter: "blur(4px)" }}
                  animate={{ opacity: 1, width: "auto", x: 0, filter: "blur(0px)" }}
                  exit={{ opacity: 0, width: 0, x: -12, filter: "blur(4px)" }}
                  transition={{
                    duration: 0.35,
                    ease: [0.16, 1, 0.3, 1],
                  }}
            ...
            <motion.div 
              layout
              className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-black shrink-0 transition-transform group-hover:scale-105"
            >
        ...
        {/* Mobile Menu Toggle */}
        <button
          className={`lg:hidden z-50 p-2 ${theme === "dark" && !isMobileMenuOpen ? "text-white" : "text-[#1a1a1a]"}`}
