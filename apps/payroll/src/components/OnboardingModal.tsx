    ...
    const checkStatus = async () => {
      ...
      try {
        ...
        if (!companyErr && company) {
          ...
          if (company.super_admin_avatar_url) setAdminPic(company.super_admin_avatar_url);
      ...
      const { data: depts } = await supabase
        .from('departments')
        .select('id, name, description, designations')
        .eq('company_id', user.id);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-[24px]">
        <div className="w-full max-w-[400px] bg-white rounded-[28px] shadow-[0_24px_48px_rgba(0,0,0,0.15)] overflow-hidden animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="px-8 py-8 flex flex-col items-center text-center">
            ...
            <div className="h-20 w-20 rounded-[20px] mb-5 flex items-center justify-center overflow-hidden shadow-lg ring-4 ring-[#007AFF]/10"
              ...
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'center', gap: 24, width: '100%', maxWidth: 480 }}>
                ...
                <div style={{ position: 'relative', flexShrink: 0, marginTop: 4 }}>
                  <div style={{
                    ...
                    display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #007AFF'
                  }}>
                    {(croppedPreviewUrl || profilePic) ? (
                      <img src={croppedPreviewUrl ?? profilePic!} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="Company" />
                    ) : (
                      <span className="avatar-initials" style={{ fontSize: 20, fontWeight: 800, color: '#007AFF' }}>
                        {getInitials(companyName || 'DA')}
                      </span>
                    )}
                  </div>
                  <div style={{
                    ...
                    <div style={{
                      ...
                      {(adminCroppedUrl || adminPic) ? (
                        <img src={adminCroppedUrl ?? adminPic!} style={{ width: '100%', height: '100%', objectFit: 'cover' }} alt="User" />
                      ) : (
                        <span className="avatar-initials" style={{ fontSize: 9, fontWeight: 800, color: '#ffffff' }}>
                          {adminFirstName
                            ? (adminFirstName[0] + (adminLastName?.[0] || '')).toUpperCase()
                            : getInitials(companyName || 'DA')}
                        </span>
                      )}
                    </div>
                        ...
                        return (
                          <div key={dept.id} className="w-full bg-[#FAFAFC] rounded-[32px] p-5 flex items-center justify-between transition-all hover:bg-[#F2F2F7] border border-[#E5E5EA]">
                            <div className="flex items-center gap-4 flex-[1.5] min-w-0">
                              <div className="w-[52px] h-[52px] rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: getAvatarColor(dept.name).bg }}>
                                <span className="font-bold text-[16px] avatar-initials" style={{ color: getAvatarColor(dept.name).color }}>{dept.name.substring(0,2).toUpperCase()}</span>
                              </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: getAvatarColor(dept.name).bg }}>
                                  <span className="font-bold text-[14px] avatar-initials" style={{ color: getAvatarColor(dept.name).color }}>{dept.name.substring(0,2).toUpperCase()}</span>
                                </div>
