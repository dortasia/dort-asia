...
export default function DepartmentSettingsPage() {
  ...
  const handleConfirmSelection = () => {
    if (activeDropdown === "dept-head") {
      const emp = employees.find(e => e.id === tempSelection);
      if (emp && emp.role !== "Admin") {
        setPendingHeadEmp(emp);
        setShowRoleTransferConfirm(true);
        return;
      }
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        if (!dept.head_id && dept.name !== "Admin Department") {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {renderSelect("Sub Approval Authority", "claims-sub", delegationConfig?.claims?.sub || headId)}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {renderSelect("Sub Approval Authority", "attendance-sub", delegationConfig?.attendance?.sub || headId)}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {renderSelect("Sub Approval Authority", "leave-sub", delegationConfig?.leave?.sub || headId)}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4">
                  ...
                  {renderSelect("Sub Approval Authority", "events-sub", delegationConfig?.events?.sub || headId)}
                </div>
              </div>
      ...
      {activeDropdown && (
        <div className="fixed inset-0 z-[9999] bg-white/90 dark:bg-black/90 flex flex-col items-center justify-center p-4">
          ...
          <div className="w-full max-w-[500px] bg-[#F4F5F7] dark:bg-[#1C1C22] rounded-[32px] p-6 shadow-none border border-white/50 dark:border-[#2C2C35]">
            ...
            {/* List of other people if searching */}
            {searchQuery.trim().length > 0 && (
              <div className="bg-white dark:bg-[#121217] rounded-[20px] overflow-hidden max-h-[160px] overflow-y-auto page-scrollbar mb-6 border border-[#E5E7EB] dark:border-[#2C2C35]">
                {employees.filter(emp => {
                  const matchesSearch = emp.name.toLowerCase().includes(searchQuery.toLowerCase());
                  if (!matchesSearch) return false;
                  if (activeDropdown === "dept-head" || activeDropdown.endsWith("-main")) {
                  ...
                  <div
                    ...
                    className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors ${tempSelection === emp.id ? 'bg-[#007AFF]/10' : 'hover:bg-gray-50 dark:hover:bg-white/5'}`}
                  >
                    <div 
                      className="h-8 w-8 rounded-full flex items-center justify-center text-[11px] font-bold border border-gray-100 dark:border-gray-800 shrink-0"
