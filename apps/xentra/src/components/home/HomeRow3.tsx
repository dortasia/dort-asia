...
function calcCompletion(emp: Record<string, any>): number {
  const fields = [
    ...
    emp.department_id, 
    emp.date_of_birth,
    emp.emergency_contact_name || emp.custom_fields?.emergency_contact_name,
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const { data: self } = await supabase.from("employees").select("company_id").eq("email", user.email).maybeSingle();
        ...
        const { data: employees, error } = await supabase
          .from("employees")
          .select("name, email, mobile, designation, department_id, date_of_birth, insurance_policy_expiry, custom_fields, bank_name, blood_group, emergency_contact_name, higher_edu_qual, avatar_url")
        ...
        const incomplete: PendingProfile[] = (employees as EmployeeRow[])
          ...
          .map(({ emp, pct }: { emp: EmployeeRow; pct: number }) => ({
            ...
            role: emp.designation || emp.designation || "Employee",
            pct,
            color: barColor(pct),
            avatarUrl: emp.avatar_url || emp.custom_fields?.profilePhotoUrl || null,
          }));
            ...
            pending.map((p, i) => (
              <div key={i} className="home-pending-card__item">
                <div className="home-pending-card__item-top">
                  {p.avatarUrl ? (
                    <img 
                      src={p.avatarUrl} 
                      alt={p.name} 
                      className="home-pending-card__avatar-circle object-cover" 
                    />
                  ) : (
                    <div
                      className="home-pending-card__avatar-circle flex items-center justify-center text-[10px] font-bold"
                      style={{ background: getAvatarColor(p.name).tint, color: getAvatarColor(p.name).solid }}
                    >
                      {p.initials}
                    </div>
                  )}
                ...
                <div className="home-pending-card__progress-row">
                  <div className="home-pending-card__bar-wrap">
                    <div
                      className="home-pending-card__bar"
                      style={{ width: `${p.pct}%`, background: p.color }}
                    />
                  </div>
                  <span className="home-pending-card__pct" style={{ color: p.color }}>
