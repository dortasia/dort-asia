...
/* ── helpers ─────────────────────────────────────────── */
function hoursFromRow(row: { clock_in?: string | null; clock_out?: string | null }): number {
  ...
  const inH = parseTime(row.clock_in);
  const outH = parseTime(row.clock_out);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        // Fetch company data safely for both super admin and employees
        ...
        const [empResult, deptResult, compResult, attendResult] = await Promise.all([
          ...
          supabase.from("company_settings").select("storage_used_gb, seat_limit").eq("company_id", companyId).maybeSingle(),
        ...
        if (attendResult.data) {
          for (const row of attendResult.data) {
            const slot = days.find((d) => d.dateStr === row.date);
            if (slot) {
