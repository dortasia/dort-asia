...
export function SettingsOvertimeHierarchyPage() {
  ...
  // React Query: Fetch settings (cached locally)
  ...
  useEffect(() => {
    async function hydrateLevels() {
      if (overtimeSettings?.hierarchy) {
        const hier = overtimeSettings.hierarchy
        ...
        if (Array.isArray(hier.approvalLevels) && hier.approvalLevels.length > 0) {
          ...
          const hydrated: HydratedApprovalLevel[] = rawLevels.map((lvl: any, idx: number) => {
            ...
            return {
              ...
              email: matchedEmp?.email || (targetId ? undefined : lvl.email),
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`company_settings_ot_hierarchy_${companyId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const saveMutation = useMutation({
    ...
    onMutate: async (newHierarchy) => {
      ...
      const updatedConfig = { ...overtimeSettings, hierarchy: newHierarchy }
      queryClient.setQueryData(['company_settings', 'overtime', companyId], updatedConfig)
    ...
    onSuccess: () => {
      toast.success('Overtime approval hierarchy saved successfully!')
    },
    onSettled: () => {
  ...
  // Normalize hierarchy before persisting: strictly store level + approverType + employeeId
    ...
    <div className="flex flex-col gap-6 max-w-[900px] pb-12">
      ...
      <div className="flex items-end justify-between">
        <div>
          <div className="flex items-center gap-2 type-small text-[#737373] mb-4">
            <Link href="/settings/overtime/advanced" className="hover:text-[#161616] transition-colors flex items-center gap-1">
          ...
          <div>
            ...
            <p className="type-small text-[#737373] mt-1">Configure multi-level approval workflows for overtime requests.</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[24px] border border-[#E5E7EB] p-8 flex flex-col gap-8 relative">
        ...
        <div>
          ...
          <p className="type-small text-[#737373] mb-4">Choose how approval paths are determined for overtime requests.</p>
