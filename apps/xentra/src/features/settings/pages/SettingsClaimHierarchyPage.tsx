...
export function SettingsClaimHierarchyPage() {
  ...
  useEffect(() => {
    async function hydrateLevels() {
      if (claimSettings?.hierarchy) {
        const hier = claimSettings.hierarchy
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`company_settings_claim_hierarchy_${companyId}`)
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  const saveMutation = useMutation({
    ...
    onMutate: async (newHierarchy) => {
      ...
      const updatedConfig = { ...claimSettings, hierarchy: newHierarchy }
      queryClient.setQueryData(['company_settings', 'claim', companyId], updatedConfig)
    ...
    onSuccess: () => {
      toast.success('Claim approval hierarchy saved successfully!')
    },
    onSettled: () => {
    ...
    <div className="flex flex-col gap-6 max-w-[900px] pb-12">
      ...
      <div className="flex items-end justify-between">
        <div>
          <div className="flex items-center gap-2 type-small text-[#737373] mb-4">
            <Link href="/settings/claims/advanced" className="hover:text-[#161616] transition-colors flex items-center gap-1">
          ...
          <div>
            ...
            <p className="type-small text-[#737373] mt-1">Configure multi-level approval workflows for claim requests.</p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[24px] border border-[#E5E7EB] p-8 flex flex-col gap-8 relative">
        ...
        <div>
          ...
          <p className="type-small text-[#737373] mb-4">Choose how approval paths are determined for claim requests.</p>
